...
class AppState:
    ...
    def export_model(_cls):
        _cls.check_active_model()
        dirpath = filedialog.askdirectory(initialdir='/', title='Select Folder', mustexist=True)
        _cls.active_model().export(dirpath)

    @classmethod
    def import_model(_cls):
        filename = filedialog.askopenfilename(defaultextension='.tar.gz', initialdir='/')
        name = filename[:-7]
        if name not in _cls.get_models():
            try:
                _cls._active_model = Model(_cls._models_dir, name, new=True, tarpath=filename)
            except Exception as e:
                raise TerpsException(f"An error occured while loading {filename}, imported file might be invalid: {str(e)}")
            _cls._state_dict['active_model'] = name
        else:
            #Should probably add some error handling for if folder is created but not model object.
            raise TerpsException(f"Model {name} already exists. Change the name of file you wish to load.")
