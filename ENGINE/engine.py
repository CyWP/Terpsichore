...
class Engine:
    ...
    def launch_mvnet(cls, task: str, err_queue: queue.Queue):
        ...
        try:
                ...
                try:
                    ...
                    try:
                        cap = cv2.VideoCapture(
                            0
                        )  # Switch to default webcam if specified index not working
                    except Exception as e:
                        raise e
            else:
            ...
            while (
                cap.isOpened()
                and not AppState.engine_aborted()
                and ret
                and not (cv2.waitKey(10) & 0xFF == ord(exit_key))
                and not (use_webcam and Timer.is_done(main_timer))
            ):
                ...
                img = tf.image.resize_with_pad(
                    ...
                    MoveNet.INFERENCE_X.value,
                    MoveNet.INFERENCE_Y.value,
                )
                ...
                try:
                    mvmt, pose, gesture_class, extr = converter.convert(
                ...
                dispatcher.dispatch(
                    mvmt.ravel(), pose.ravel(), gesture_class, extr.ravel()
                )
