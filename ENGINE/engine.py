...
class Engine():
    ...
    _momentum = 0.2 #number of previous frames for which delta pose data is recorded
    ...
    #def launch(_cls, webcam, webindex, vidpath, tfpath, wait, duration, show):
    #Load Model
        interpreter = tf.lite.Interpreter(model_path=_cls._tf_model_path)
        interpreter.allocate_tensors()

        props = AppState.get_ui_inputs()

        dispatcher = select_dispatcher(task, props)

        #Set Input device
        if props['use_webcam']:
            try:
                cap = cv2.VideoCapture(props['webcam_index'])
            except:
                cap = cv2.VideoCapture(0)
        else:
            cap = cv2.VideoCapture(props['video_path'])
